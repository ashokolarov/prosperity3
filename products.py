...
class RainforestResin(Product):
    def __init__(self):
        ...
        self.liquidation_profit_margin = 4
    ...
    def market_take(self, timestamp):
        ...
        mt_ask_orders = []
        for depth_level in range(len(self.order_book.bid_prices)):
    ...
    def liquidate_mt_orders(self):
        orders = []
        for pos_price, pos_volume, timestamp in self.mt_positions.positions:
            best_ask, best_ask_volume = self.order_book.best_ask
            best_bid, best_bid_volume = self.order_book.best_bid

            if pos_volume > 0:
                if best_ask - pos_price > self.liquidation_profit_margin:
                    qty = min(pos_volume, best_ask_volume)
                    ask_order = Order(self.symbol, best_ask, -qty)
                    orders.append(ask_order)
                    self.order_book.update(ask_order)
                    self.mt_positions.liquidate_pos(qty, timestamp)
            elif pos_volume < 0:
                if pos_price - best_bid > self.liquidation_profit_margin:
                    qty = min(-pos_volume, best_bid_volume)
                    bid_order = Order(self.symbol, best_bid, qty)
    ...
    def calculate_orders(self, order_depths, position, timestamp):
        ...
        mt_orders = self.market_take(timestamp)
        liquid_orders = self.liquidate_mt_orders()
