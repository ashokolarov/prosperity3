...
class MagnificentMacarons(Product):
    def __init__(self, config):
        ...
        self.z_open = config.get("z_open")
        self.z_close = config.get("z_close")
    ...
    def directional(self):
        ...
        self.logger.print_numeric("mean", mean)
        self.logger.print_numeric("std", std)

        z_score = abs(self.fair_value - mean) / std
        self.logger.print_numeric("z_score", z_score)

        if z_score > self.z_open and self.remaining_buy > 0:
            # Place orders based on the z-score
            best_bid_price, best_bid_volume = self.order_book.get_best_ask()
            self.place_order(best_bid_price, -best_bid_volume)
        elif z_score < self.z_open:
            # Place orders based on the z-score
            best_ask_price, best_ask_volume = self.order_book.get_best_bid()
            self.place_order(best_ask_price, best_ask_volume)
        elif z_score < self.z_close and self.remaining_sell > 0:
            # Place orders based on the z-score
            best_bid_price, best_bid_volume = self.order_book.get_best_ask()
            self.place_order(best_bid_price, -best_bid_volume)
        elif z_score > self.z_close and self.remaining_sell > 0:
